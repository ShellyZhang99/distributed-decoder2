		    ...
		    struct pt_image *image, char *arg, const char *prog)
{
    ...
    errcode = preprocess_filename("test", &foffset, &fsize);
    ...
    if (!fsize)
        fsize = UINT64_MAX;
    isid = pt_iscache_add_file(iscache, "test", foffset, fsize, base);
    if (isid < 0) {
...
static int pt2_init_decoder(struct pt2_decoder *decoder)
{
    ...
    for (int i = 0; i < PARALLEL_LEVEL; i++){
        ...
        struct pt_image *image;
            ...
            const char *argc = "test\0";

        excl_decoder->iscache = pt_iscache_alloc(NULL);
       /* int errcode = load_raw(excl_decoder->iscache, image, argc, "pt2_decoder");
                			if (errcode < 0) {
                			...
                			/*uint64_t base = 0ull;
                            			int errcode = extract_base(argc, &base);

                            			errcode = load_elf(excl_decoder->iscache, image, "test", base,
                            					   "pt2_decoder", 0);*/
...
static int split_trace(struct pt2_decoder *decoder,
    ...
    uint64_t *offset_buf = malloc( 8 * size );
    if (!offset_buf){
    ...
    for (;;) {
        ...
        if (cnt+1 >= size){
            ...
            uint64_t *new_buf = malloc(8*new_size);
            if (!new_buf){
                free(offset_buf);
                fprintf(stderr, "failed to create offset buffer\n");
                return -pte_nomem;
